...
def combo():
    with open("combo.in","r") as file:
        N=int(file.readline())
        key1=list(map(int,file.readline().strip().split(" ")))
        key2=list(map(int,file.readline().strip().split(" ")))
        res=set()
        temp1=[[]]
        for i in range(len(key1)):
            ...
            for item in temp1:
                for shift in range(3):
                    if shift ==0:
                        temp.append(item+[key1[i]])
                    else:
                        temp.append(item+[(key1[i]+shift+49)%50+1])
                        temp.append(item+[(key1[i]-shift+49)%50+1])
            temp1=temp
        temp2=[[]]
        for i in range(len(key2)):
            ...
            for item in temp2:
                for shift in range(3):
                    if shift ==0:
                        temp.append(item+[key2[i]])
                    else:
                        temp.append(item+[(key2[i]+shift+49)%50+1])
                        temp.append(item+[(key2[i]-shift+49)%50+1])
            temp2=temp
